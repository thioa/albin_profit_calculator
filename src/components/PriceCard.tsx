...
export default function PriceCard({ price, isPremium, isLowest, buyPrice, isBuyCity, onSetBuyCity }: PriceCardProps) {
  ...
  const getFreshnessUI = (level: string) => {
    switch (level) {
      ...
      default: return { color: "text-primary/50", label: "Unknown", icon: ShieldAlert };
    }
  };
  ...
  return (
    <div className={cn(
      "glass-panel border rounded-2xl p-6 transition-all duration-300 hover:scale-[1.02] border-t-4",
      getFreshnessBorder(freshness),
      isBuyCity ? "border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.15)]" : isLowest ? "border-primary shadow-[0_0_20px_rgba(212,175,55,0.15)]" : "border-primary/10"
    )}>
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-2">
          <MapPin className={cn("w-4 h-4", isBuyCity ? "text-blue-500" : "text-primary")} />
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {isLowest && !isBuyCity && (
            <span className="bg-primary text-black text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">
              Lowest Price
            </span>
          )}
        </div>
      ...
      {!hasData ? (
          ...
          {price.historical_avg ? (
            <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4 space-y-2">
              ...
              <div className="flex justify-between items-end">
                <div>
                  <div className="text-[10px] text-primary/50 uppercase tracking-wider">Avg Price</div>
                  <div className="text-xl font-mono font-bold text-on-surface">
                    {formatSilver(price.historical_avg)}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[10px] text-primary/50 uppercase tracking-wider">Volume</div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="text-xs text-primary/60 uppercase tracking-wider mb-1">Min Sell</div>
              <div className="text-2xl font-mono font-bold text-white">
                {formatSilver(price.sell_price_min)}
              </div>
            </div>
            <div>
              <div className="text-xs text-primary/60 uppercase tracking-wider mb-1">Max Buy</div>
          ...
          {price.historical_avg && (
            <div className="flex justify-between items-center bg-white/5 px-3 py-2 rounded-lg border border-white/5">
              <div className="text-[9px] text-primary/50 uppercase tracking-wider">Historical Avg</div>
              <div className="text-xs font-mono text-primary/60">{formatSilver(price.historical_avg)}</div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="text-xs text-primary/60 uppercase tracking-wider mb-1">Net Sale</div>
              <div className="text-lg font-mono font-bold text-gray-200">
                {formatSilver(profit.netProfit)}
              </div>
            </div>
            <div className="text-right">
              <div className="text-[10px] text-primary/50 uppercase tracking-wider mb-1">Tax ({isPremium ? '4%' : '8%'})</div>
              <div className="text-xs text-primary/60">-{formatSilver(profit.totalFees)}</div>
            </div>
          </div>
          ...
          <div className="bg-black/20 p-4 rounded-xl border border-white/5">
            <div className="flex justify-between items-center">
              <div className="space-y-1">
                <div className="text-xs text-primary/60 uppercase tracking-wider">Final Profit</div>
                {isProfitable && (
              ...
              <div className={cn(
                ...
                isProfitable ? "text-green-500" : profit.finalProfit < 0 ? "text-red-500" : "text-primary/60"
              )}>
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-primary/10/50">
            <div className="flex items-center gap-2">
