...
const SUB_CONFIG = {
  ...
  free: { label: 'Free', icon: Zap, color: 'text-sidebar-foreground/75', bg: 'bg-sidebar-accent/30', border: 'border-sidebar-border/20' },
};
...
export default function ProfileView({
  ...
  if (!user) return (
    <div className="flex flex-col items-center justify-center p-20 text-center space-y-6 glass-panel rounded-3xl border border-sidebar-border/20">
      ...
      <div>
        ...
        <p className="text-sidebar-foreground/75 max-w-sm mx-auto mt-2 text-sm leading-relaxed">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel p-6 rounded-3xl border border-sidebar-border/20 relative overflow-hidden">
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6">
          ...
          <div className="relative shrink-0 mx-auto lg:mx-0">
            ...
            <div className={`absolute -bottom-2 -right-2 flex items-center gap-1.5 px-2 py-1 rounded-lg border text-label font-bold uppercase tracking-wider shadow-lg ${roleCfg.bg} ${roleCfg.border} ${roleCfg.color}`}>
          ...
          <div className="flex-1 w-full">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-sidebar-foreground/75 mb-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={stat.label} className="bg-sidebar-accent/30 rounded-xl p-3 text-center">
                  <p className="text-label text-sidebar-foreground/70 uppercase font-bold tracking-wider">{stat.label}</p>
          ...
          <div className="flex flex-row lg:flex-col gap-2 w-full lg:w-auto shrink-0">
            <button onClick={logout}
              className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-destructive/10 hover:bg-destructive/20 text-destructive border border-destructive/20 rounded-xl font-bold uppercase text-label tracking-wider transition-all">
              <LogOut className="w-3.5 h-3.5" /> Sign Out
            </button>
            <button className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-sidebar-accent/50 hover:bg-sidebar-accent text-sidebar-foreground border border-sidebar-border rounded-xl font-bold uppercase text-label tracking-wider transition-all">
      ...
      {/* â”€â”€ Subscription Banner (free users only) â”€â”€ */}
      {!isPremium && (
        <div className="glass-panel p-4 rounded-2xl border border-yellow-500/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-sidebar-foreground/75">Unlimited crafting plans Â· Unlimited watchlist Â· Priority scan</p>
            </div>
          </div>
          <button
            onClick={upgradeToPremium}
            className="flex items-center gap-2 px-4 py-2 bg-yellow-500/15 hover:bg-yellow-500/25 text-yellow-400 border border-yellow-500/30 rounded-xl font-bold uppercase text-label tracking-wider transition-all shrink-0"
          >
        ...
        {[
          ...
          return (
            <button key={tab.id} onClick={() => setActiveSubTab(tab.id as any)}
              className={`relative flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${activeSubTab === tab.id ? 'bg-sidebar-primary text-sidebar-primary-foreground shadow-sm' : 'text-sidebar-foreground/75 hover:bg-sidebar-accent hover:text-sidebar-foreground'}`}>
              <Icon className="w-3.5 h-3.5" /> {tab.label}
              {tab.badge ? <span className="absolute -top-1 -right-1 w-4 h-4 bg-destructive rounded-full text-micro font-black flex items-center justify-center text-white">{tab.badge}</span> : null}
            </button>
          );
        })}
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeSubTab === 'sims' && (
            <motion.div key="sims" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              <h4 className="text-xs font-bold text-sidebar-foreground/70 uppercase tracking-wider">Saved Simulations</h4>
                  ...
                  {user.savedSimulations.map(sim => (
                    <div key={sim.id} className="group p-4 bg-sidebar-accent/20 border border-sidebar-border/20 rounded-xl hover:border-sidebar-primary/30 transition-all">
                      <div className="flex items-center justify-between mb-2">
                        <span className="px-2 py-0.5 rounded text-label font-bold uppercase tracking-wider bg-sidebar-accent/50 text-sidebar-foreground/70">
                          {sim.type}
                        </span>
                        <span className="text-label text-sidebar-foreground/30">{formatTimeAgo(sim.timestamp)}</span>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => onSelectSimulation(sim)} className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-sidebar-primary/10 hover:bg-sidebar-primary text-sidebar-primary hover:text-sidebar-primary-foreground rounded-lg text-label font-bold uppercase tracking-wider transition-all">
          ...
          {activeSubTab === 'watch' && (
            <motion.div key="watch" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              <h4 className="text-xs font-bold text-sidebar-foreground/70 uppercase tracking-wider">Watched Items ({user.watchlist.length})</h4>
              {!user.watchlist.length ? (
                <div className="flex flex-col items-center justify-center py-16 opacity-40 gap-3">
                  ...
                  <p className="text-xs text-sidebar-foreground/70">Add items from Top Flips to watch them.</p>
                </div>
              ) : (
                  ...
                  {user.watchlist.map(id => (
                    <div key={id} className="p-3 bg-sidebar-accent/20 border border-sidebar-border/20 rounded-lg flex items-center gap-3 group hover:border-sidebar-primary/30 transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sidebar-foreground/30 text-label uppercase tracking-wider mt-0.5">Live Tracking</p>
                      </div>
          ...
          {activeSubTab === 'notifs' && (
            <motion.div key="notifs" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-sidebar-foreground/70 uppercase tracking-wider">Alerts & System Logs</h4>
                {notifications.length > 0 && (
                  <button onClick={clearNotifications} className="flex items-center gap-1.5 text-label text-sidebar-foreground/70 hover:text-sidebar-foreground transition-colors font-bold uppercase tracking-wider">
                    ...
                    <div key={n.id} onClick={() => markAsRead(n.id)}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-label text-sidebar-foreground/30 shrink-0 ml-2">{formatTimeAgo(n.timestamp)}</span>
                        </div>
                        <p className="text-sidebar-foreground/75 text-xs leading-relaxed mt-1">{n.message}</p>
                      </div>
                      {!n.read && <div className="w-2 h-2 bg-sidebar-primary rounded-full shadow-glow-primary-bright mt-1.5 shrink-0" />}
                    </div>
                  ))}
                </div>
              )}
