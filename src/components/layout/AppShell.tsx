...
import MarketOpportunities, { VerificationStatus, SortOption } from "../features/MarketOpportunities";
import HighValueSales from "../features/HighValueSales";
...
import CraftingRecommendations from "../features/CraftingRecommendations";
...
export default function AppShell({ server, onServerChange }: { server: AlbionServer; onServerChange?: (s: AlbionServer) => void }) {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 h-screen w-64 z-40 bg-[#0a0e14] flex flex-col pt-20 pb-8 bg-linear-to-b from-primary/10 to-transparent">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <button onClick={() => setActiveTab("notifications")} className={`w-full flex items-center justify-between gap-4 py-3 px-6 font-headline text-sm uppercase tracking-widest transition-all ${activeTab === "notifications" ? "text-secondary border-l-4 border-secondary bg-secondary/5 font-bold" : "text-primary/60 hover:text-secondary hover:bg-primary/5"}`}>
            ...
            {unreadNotifications > 0 && (
              <span className="bg-error text-white text-xs font-black px-2 py-0.5 rounded-full animate-pulse min-w-5 text-center">{unreadNotifications}</span>
            )}
          </button>
      ...
      <main className="flex-1 ml-64 min-h-screen pb-12 w-[calc(100%-16rem)] overflow-x-hidden">
        <header className="fixed top-0 right-0 left-64 z-50 bg-[#0a0e14]/60 backdrop-blur-xl border-b border-primary/15 flex justify-between items-center px-6 h-16 shadow-[0_8px_32px_rgba(15,82,186,0.08)]">
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-headline font-bold text-secondary tracking-widest uppercase truncate max-w-50 md:max-w-none">Albion Navigator</h1>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 border-l border-primary/15 pl-6">
              <button
                ...
                className="text-primary/70 hover:bg-primary/10 p-2.5 rounded-lg transition-colors active:scale-95 min-w-11 min-h-11 flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-primary/50"
              >
        ...
        <div className="mt-16 p-8 max-w-7xl mx-auto space-y-8">
          ...
          {activeTab !== "crafting" && activeTab !== "refining" && activeTab !== "cooking" && activeTab !== "library" && activeTab !== "profile" && activeTab !== "notifications" && activeTab !== "my-crafting" && (
            <div className="flex flex-col items-center gap-3 w-full">
              {/* PRIMARY FILTERS ROW */}
              <div className="flex flex-wrap justify-center items-center gap-3 text-sm w-full">
                <CityFilter selectedCities={selectedCities} onChange={setSelectedCities} />

              <div className="flex items-center gap-3 glass-panel p-2 rounded-xl border border-primary/10 h-12">
                <Sparkles className="w-5 h-5 text-primary" />
                <span className="text-xs font-bold text-primary/50 uppercase tracking-wider">Enchant</span>
                  ...
                  {[0, 1, 2, 3, 4].map((level) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={`Enchantment level ${level}`}
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="bg-primary text-black px-2 py-0.5 rounded text-xs font-bold">{selectedQualities.length}</span>
                <ChevronDown className={`w-5 h-5 transition-transform ${showQualityFilter ? 'rotate-180' : ''}`} />
              </button>
            ...
            {activeTab === "search" && (
              <div className="flex items-center gap-3 glass-panel p-2 rounded-xl border border-primary/10 h-12">
                <span className="text-primary/50 font-medium px-2 text-sm">Buy Price:</span>
                <input
                  type="number"
                  value={buyPrice || ""}
                  onChange={(e) => setBuyPrice(Number(e.target.value))}
                  placeholder="0"
                  className="bg-transparent text-white focus:outline-none w-28 font-mono text-right text-sm"
                />
              </div>
            )}
              </div>
              ...
              {(activeTab === "opportunities" || activeTab === "high-value" || activeTab === "recommendations") && (
              <div className="flex flex-wrap justify-center items-center gap-3 text-sm mt-1 w-full">
                <div className="relative">
                  <button
                  ...
                  >
                    ...
                    <ChevronDown className={`w-5 h-5 transition-transform ${showSortFilter ? 'rotate-180' : ''}`} />
                  </button>
                      ...
                      <div className="absolute top-full mt-2 left-0 w-48 glass-panel rounded-xl shadow-2xl z-50 p-2 overflow-hidden">
                        {[
                          ...
                          <button
                            ...
                            className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-white/5 transition-colors group min-h-12"
                          >
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown className={`w-5 h-5 transition-transform ${showStatusFilter ? 'rotate-180' : ''}`} />
                  </button>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="bg-primary text-black px-2 py-0.5 rounded text-xs font-bold">{selectedCategories.includes("All") ? "All" : selectedCategories.length}</span>
                    <ChevronDown className={`w-5 h-5 transition-transform ${showCategoryFilter ? 'rotate-180' : ''}`} />
                  </button>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="bg-primary text-black px-2 py-0.5 rounded text-xs font-bold">{selectedSubCategory}</span>
                    <ChevronDown className={`w-5 h-5 transition-transform ${showSubFilter ? 'rotate-180' : ''}`} />
                  </button>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="bg-primary text-black px-2 py-0.5 rounded text-xs font-bold">
                      {maxAgeHours === 0 ? "Any" : `${maxAgeHours}h`}
                    </span>
                    <ChevronDown className={`w-5 h-5 transition-transform ${showMaxAgeFilter ? 'rotate-180' : ''}`} />
                  </button>
